        ...
        with open(test_result_path, "w") as f2:
            ...
            for epoch in range(EPOCH):
                for i, data in enumerate(train_loader):  # gives batch data, normalize x when iterate train_loader
                    ...
                    precision = precision_score(label, y_predicted, average='weighted')
                    recall = recall_score(label, y_predicted, average='weighted')
                ...
                print(np.mean(test_precision))
                print(np.mean(test_recall))
                ...
                with torch.no_grad():  # 无梯度
                    ...
                    for i, data in enumerate(test_loader):
                        ...
                        f2.write('\n')
                        f2.flush()
            torch.save(model.state_dict(), 'model/1_model_epoch_{}.pth'.format(epoch + 1))
            torch.save(model, 'model/1_sturacture.pth')
...
if __name__ == "__main__":
    ...
    modeltrian(tp=name, EPOCH=200, LR=0.0001, test_ratio=0.319,
                                   start=0, end=2048, ncls=4, psize=8, depth=3, heads=4, mlp_dim=1024,
